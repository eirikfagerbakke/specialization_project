...
from jax import random, vmap, grad, custom_vjp
...
class FNOTimeStepping(AbstractOperatorNet):
    ...
    def Dxx(self, a, x, t):
        def step(v, _):
            ...
            d_Q_dv_L = grad(lambda v_L : jnp.sum(self.projection(self.activation(v_L))))(v_L) # 1st derivative
            d2_Q_dv_L2 = grad(grad(lambda v_L : jnp.sum(self.projection(self.activation(v_L)))))(v_L) # 2nd derivative
            
            d_K_L_dx = self.last_spectral_conv.Dx(v_Lm1, x[1]-x[0])
            d2_K_L_dx2 = self.last_spectral_conv.Dxx(v_Lm1, x[1]-x[0])
            
            v = self.projection(self.activation(v_L))[0]
            return v, (d2_Q_dv_L2 * (d_K_L_dx)**2 + d2_K_L_dx2*d_Q_dv_L).sum(axis=0)
    ...
    def Dxxx(self, a, x, t):
        def step(v, _):
            ...
            d_Q_dv_L = grad(lambda v_L : jnp.sum(self.projection(self.activation(v_L))))(v_L) # 1st derivative
            d2_Q_dv_L2 = grad(grad(lambda v_L : jnp.sum(self.projection(self.activation(v_L)))))(v_L) # 2nd derivative
            d3_Q_dv_L3 = grad(grad(grad(lambda v_L : jnp.sum(self.projection(self.activation(v_L))))))(v_L) # 3rd derivative
            
            d_K_L_dx = self.last_spectral_conv.Dx(v_Lm1, x[1]-x[0])
            d2_K_L_dx2 = self.last_spectral_conv.Dxx(v_Lm1, x[1]-x[0])
            d3_K_L_dx3 = self.last_spectral_conv.Dxxx(v_Lm1, x[1]-x[0])
            
            v = self.projection(self.activation(v_L))[0]
            return v, (d3_Q_dv_L3*d_K_L_dx**3 + 3*d2_Q_dv_L2*d_K_L_dx*d2_K_L_dx2+ d3_K_L_dx3*d_Q_dv_L).sum(axis=0)
    ...
    def u_xx(self, a, x, t):
        return self.Dx(a, x, t)*self.u_std/self.x_std**2
    
    def u_xxx(self, a, x, t):
        return self.Dx(a, x, t)*self.u_std/self.x_std**3
    
    def u_x_whole_grid(self, a, x, t):
        return self.Dx_whole_grid(a, x, t)*self.u_std/self.x_std

@partial(custom_vjp, nondiff_argnums=(0,1))
def _call(model, a, x, t):
    print("Calling _call")
    def step(v, _):
        # combine with spatial grid
        v_prev = v
        v = model.stack_input(v, x)
                    
        v = model.lifting(v)
        
        def f(v, dynamic_fno_block):
            fno_block = eqx.combine(dynamic_fno_block, model.static_fno_blocks)
            return fno_block(v), None
        
        v, _ = jax.lax.scan(f, v, model.dynamic_fno_blocks)
        
        v = model.activation(model.last_spectral_conv(v) + model.last_bias)
        v = model.projection(v)[0]
        
        return v, v_prev
        
    _, out_max = jax.lax.scan(step, a, length = model.max_steps)

    t_max = model.encode_t(jnp.linspace(0, 2, model.max_steps))
    out = interp1d(t, t_max, out_max, method="akima")
    return out

def _call_fwd(model, a, x, t):
    print("Calling _call_fwd")
    def step(v, _):
        # combine with spatial grid
        v_prev = v
        v = model.stack_input(v, x)
        ...
        v_Lm1, _ = jax.lax.scan(f, v, model.dynamic_fno_blocks)
        
        v_L = model.last_spectral_conv(v_Lm1) + model.last_bias
        
        #d_Q_d_v_L = grad(lambda v_L : jnp.sum(model.projection(model.activation(v_L))))(v_L)
        #d_K_L_dx = model.last_spectral_conv.Dx(v_Lm1, x[1]-x[0])
        
        u = model.projection(model.activation(v_L))[0]
        #return v, (v_prev, (d_Q_d_v_L * d_K_L_dx).sum(axis=0))
        return u, (v_prev, (v_Lm1, v_L)) 
        
    _, (u_at_timepoints, (v_Lm1s, v_Ls)) = jax.lax.scan(step, a, length = model.max_steps)

    timepoints = model.encode_t(jnp.linspace(0, 2, model.max_steps))
    out = interp1d(t, timepoints, u_at_timepoints, method="akima")
    return out, (v_Lm1s, v_Ls, x, t, timepoints, u_at_timepoints)

def _call_bwd(model, a, res, g):
    print("Calling _call_bwd")
    v_Lm1s, v_Ls, x, t, timepoints, u_at_timepoints = res
    dx = x[1]-x[0]
    def step(v_Lm1, v_L):
        
        print(v_Lm1.shape, v_L.shape)

        d_Q_d_v_L = grad(lambda v_L : jnp.sum(model.projection(model.activation(v_L))))(v_L)
        d_K_L_dx = model.last_spectral_conv.Dx(v_Lm1, dx)

        return (d_Q_d_v_L * d_K_L_dx).sum(axis=0)
        
    u_x_at_timepoints = vmap(step)(v_Lm1s, v_Ls)
    u_x_interp = interp1d(t, timepoints, u_x_at_timepoints, method="akima")
    u_t_interp = interp1d(t, timepoints, u_at_timepoints, method="akima", derivative = 1)
    return (u_x_interp * g, u_t_interp * g)
