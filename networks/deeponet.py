...
class DeepONet(AbstractOperatorNet):
    ...
    def __init__(self, hparams : Union[Hparams, dict]):
        ...
        self.branch_net = init_he(eqx.nn.MLP(
            in_size = hparams.number_of_sensors, # input is [cos(2πx/P), sin(2πx/P), t]
...
def compute_loss(model, a, u, key):
    ...
    Args:
        ...
        a (batch, M+1): The input data.
        u (batch, N+1, M+1): The ground truth data at the query points.
    ...
    # Has shape (batch_size, num_query_points)
    branch_out = vmap(model.eval_branch)(a) # (batch, p)
    trunk_out = vmap(vmap(model.eval_trunk))(Trainer.x[x_idx], Trainer.t[t_idx]) # (batch, num_points, p)
    u_pred = jnp.einsum('bp,bnp->bn', branch_out, trunk_out) + model.bias[0]
    ...
    trunk_at_t_zero = vmap(model.eval_trunk, (0, None))(Trainer.x, Trainer.t[0]) # (M+1, p)
    a_pred = jnp.einsum('bp,mp->bm', branch_out, trunk_at_t_zero) + model.bias[0] # (batch, M+1)
